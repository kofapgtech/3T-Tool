 ...
 * a Social Value figure. This mirrors that: no multi-select simulator.
 * Net Value Created is a plain derived figure (Social Value − Program
 * Spend), not a separate modeled quantity — there's no TVO input here.
 */
export default function Sandbox() {
  ...
  function handleSaveClick() {
    if (!session) {
      navigate('/login', {
        state: { from: '/sandbox', message: 'Create an account or sign in to save this scenario.' },
      })
      return
    }
    setSaveError(null)
...
function SandboxFooter() {
  return (
    <footer className="rounded-lg bg-ink px-6 py-6">
